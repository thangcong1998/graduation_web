...
class CongressRecordController extends ApiResourceController
{
    ...
    public function deleteList(Request $request)
    {
        try {
            ...
            if ($request->action == 'force') {
                ...
                $delete = CongressRecord::whereIn('id', $id)->forceDelete();
                DB::commit();
                return $this->deleteResultResponse($delete);
            } else {
                $id = json_decode($request->ids);

                $delete = CongressRecord::whereIn('id', $id)->delete();
                DB::commit();
                return $this->deleteResultResponse($delete);
            }
        } catch (\Exception $e) {
    ...
    public function exportData(Request $request)
    {
        ...
        foreach ($record as $key => $value) {
            ...
            if ($data[$key]['unit'] == CongressRecord::unit['hour']) {
                $unit = Lang::get("constants.unit.hour");
            }
            if ($data[$key]['unit'] == CongressRecord::unit['minute']) {
                $unit = Lang::get("constants.unit.minute");
            }
            if ($data[$key]['unit'] == CongressRecord::unit['seconds']) {
                $unit = Lang::get("constants.unit.seconds");
            }
            if ($data[$key]['unit'] == CongressRecord::unit['kilometer']) {
                $unit = Lang::get("constants.unit.kilometer");
            }
            if ($data[$key]['unit'] == CongressRecord::unit['meter']) {
                $unit = Lang::get("constants.unit.meter");
            }
            if ($data[$key]['unit'] == CongressRecord::unit['centimeter']) {
                $unit = Lang::get("constants.unit.centimeter");
            }
            if ($data[$key]['unit'] == CongressRecord::unit['match']) {
                $unit = Lang::get("constants.unit.match");
            }
            if ($data[$key]['unit'] == CongressRecord::unit['goal']) {
                $unit = Lang::get("constants.unit.goal");
            }
            $data[$key]['unit'] = $unit;
        }
        return Excel::download(new DataExport($id, $header, $data), 'CongressRecord.xlsx');
    }
}
