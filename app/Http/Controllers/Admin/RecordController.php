...
class RecordController extends ApiResourceController
{
    ...
    public function exportData(Request $request)
    {
        ...
        $data = [];
        $record = $this->query->whereIn('id', $id)->with([
            'event', 'event.sportDiscipline',
            'event.sportDiscipline.sport', 'competitor', 'competitor.nationality', 'country'
        ])->get();
        // return $record;
        foreach ($record as $key => $value) {
            ...
            foreach ($fields as $field) {
                ...
                // if ($field == 'country') {
                //     $data[$key]['country'] && $value['country'] == null ? $data[$key]['country'] = $value['competitor_id'] :
            ...
            if ($data[$key]['is_seagame'] == 2) {
                $is_seagame = Lang::get('constants.uniform.yes');
            } else {
                $is_seagame = Lang::get('constants.uniform.no');
            }
            $data[$key]['is_seagame'] = $is_seagame;

            if ($data[$key]['is_asiad'] == 2) {
                $is_asiad = Lang::get('constants.uniform.yes');
            } else {
                $is_asiad = Lang::get('constants.uniform.no');
            }
            $data[$key]['is_asiad'] = $is_asiad;

            if ($data[$key]['is_olympic'] == 2) {
                $is_olympic = Lang::get('constants.uniform.yes');
            } else {
                $is_olympic = Lang::get('constants.uniform.no');
            }
            $data[$key]['is_olympic'] = $is_olympic;
            if ($data[$key]['unit'] == Record::unit['hour']) {
                $unit = Lang::get("constants.unit.hour");
            }
            if ($data[$key]['unit'] == Record::unit['minute']) {
                $unit = Lang::get("constants.unit.minute");
            }
            if ($data[$key]['unit'] == Record::unit['seconds']) {
                $unit = Lang::get("constants.unit.seconds");
            }
            if ($data[$key]['unit'] == Record::unit['kilometer']) {
                $unit = Lang::get("constants.unit.kilometer");
            }
            if ($data[$key]['unit'] == Record::unit['meter']) {
                $unit = Lang::get("constants.unit.meter");
            }
            if ($data[$key]['unit'] == Record::unit['centimeter']) {
                $unit = Lang::get("constants.unit.centimeter");
            }
            if ($data[$key]['unit'] == Record::unit['match']) {
                $unit = Lang::get("constants.unit.match");
            }
            if ($data[$key]['unit'] == Record::unit['goal']) {
                $unit = Lang::get("constants.unit.goal");
            }
            $data[$key]['unit'] = $unit;
        }
        return Excel::download(new DataExport($id, $header, $data), 'record.xlsx');
    }
}
