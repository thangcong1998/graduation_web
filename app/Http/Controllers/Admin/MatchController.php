...
class MatchController extends ApiResourceController
{
    ...
    public function update(MatchUpdateRequest $request, $id)
    {
        try {
            ...
            $event_cp_type = $stage['event']->competition_type;
            if (isset($request->event_group_id)) {
                if($event_cp_type == SportDisciplineEvent::competition_type['INDIVIDUAL']) {

                    $mic_ids = [];
                    foreach($request->competitors as $val) {
                        $mic = MatchIndividualCompetitor::query()->updateOrCreate(
                            [
                                'match_id' => $id,
                                'competitor_id' => $val['competitor_id'],
                                'event_group_member_id' => $val['event_group_member_id']
                    ...
                    DB::table('match_individual_competitors')->where('match_id', $id)->whereNotIn('id', $mic_ids)->delete();
                }
                if($event_cp_type == SportDisciplineEvent::competition_type['TEAM']) {
                    $met_ids = [];
                    foreach($request->competitors as $val) {
                        $met = MatchEventTeam::query()->updateOrCreate([
                            'match_id' => $id,
                            'event_team_id' => $val['competitor_id'],
                            'event_group_member_id' => $val['event_group_member_id']
                        ]);
                    ...
                    DB::table('match_event_teams')->where('match_id', $id)->whereNotIn('id', $met_ids)->delete();
                }
            } else {
                if($event_cp_type == SportDisciplineEvent::competition_type['INDIVIDUAL']) {
                    $mic_ids = [];
            ...
            if($request->competition_type == 1){
                ...
                if($request->member1['competitor_id']){
                    MatchIndividualCompetitor::query()->where("match_id",$request->match_id)
                        ...
                        "final_score"=>isset($request->final_scoreA) ? $request->final_scoreA : 0
                    ]);
                }
                if($request->member2['competitor_id']){
                    ...
                    ->where("competitor_id",$request->member2['competitor_id'])->update([
                        ...
                        "final_score"=>isset($request->final_scoreB) ? $request->final_scoreB : 0
                    ]);
                }
            }else{
                ...
                if($request->competition_type == 1){
                    if($member1['competitor_id']){
                        ...
                        if($request->qualificationA == 0){
                            StageQualificationCompetitor::query()->where("stage_id",$request->stage_id)
                            ->where("participant_id",$member1['competitor_id'])->delete();
                        }
                    }
                    if($member2['competitor_id']){
                        ...
                        if($request->qualificationB == 0){
                            StageQualificationCompetitor::query()->where("stage_id",$request->stage_id)
                            ->where("participant_id",$member2['competitor_id'])->delete();
                        }
                    };
                }else {
    ...
    {
        $match = Match::query()->where("id",$id)->first();
        $stage_id = $match->stage_id;
        $this->query->with([ 'competitors.stage_qualification_competitors'=>function($q) use($stage_id){
            return  $q->where("stage_id",$stage_id)->get();
        },]);
    ...
    public function addAppend()
    {
        $this->query->with([
            ...
            'event_teams.stage_qualification_competitors',
            'event_teams.team',
    ...
    public function uploadFile(Request $request)
    {
        try {
            DB::beginTransaction();
            $item = Match::query()->where("id",$request->match_id)->first();
            ...
            if ($request->hasFile('files')) {
                $files = $request->file();
                 $ac= $this->upload($files, $item, "report_match");
            }
            DB::commit();
    ...
    protected function upload($files, $model, $url)
    {
        foreach ($files['files'] as $file) {
            $path = Storage::disk('public')->put($url, $file);
            ...
            Files::query()->where("id",$attachment->id)->update([
                'code'=>$code.$attachment->id,
            ]);
