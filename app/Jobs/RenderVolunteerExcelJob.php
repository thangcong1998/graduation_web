...
class RenderVolunteerExcelJob implements ShouldQueue
{
    ...
    function updateExcelVolunteer()
    {
        try {

            $objPHPExcel = IOFactory::load('public/storage/documents/VolunteerForm.xlsx');
            ...
            $ors = Organization::query()->where('is_holder', '=', 2)->with(['functions' => function ($query) {
                $query->where('is_volunteer', '=', 2);
            }])->get();
            // $rowFunction = 2;
            // $column = 'A';
            // $lastFunctionRow = $sheet->getHighestRow();
            // $functionName = [];
            // $functions = Functions::query()->where('is_volunteer', '=', 2)->whereHas('Organization', function (Builder $query) {
            ...
            $objWriter->save('public/storage/documents/VolunteerForm.xlsx');
            return 'ok';
        } catch (Exception $e) {
            throw $e;
