...
export default function RoleForm () {
    ...
    const { data: permission, loading: loading, revalidate: refetch} = useFetch(['get', '/admin/all-permissions']);
    const { data: groupName, loading: loading3 } = useFetch(['get', '/admin/roleGroupName']);
    const {data: fetcher2, revalidate: refetch2, loading: loadingApi2,} = useFetch(
    ...
    return (
        <Paper style={{ padding: 10}}>
            <Grid container spacing={3}>
                ...
                <Grid item xs={3} style={{ borderRight: '#f3f3f3 solid 1px', padding: 10}}>
                    {arrayGroup.map(value => {
                        ...
                        return (
                            <div
                                style={ value.group == showGroup ?
                                    { minHeight: '50px', width: '100%', margin: '5px 0px', borderBottom: '#f3f3f3 solid 1px', backgroundColor: '#fcefe5', padding: 10} :
                                    { minHeight: '50px', width: '100%', margin: '5px 0px', borderBottom: '#f3f3f3 solid 1px', padding: 10}
                                }
                                onClick={() => setShowGroup(value.group)}>
                                <p style={{ fontWeight: "bold", fontSize: '15px', margin: 0}}>{i18n.language == "vi" ? value.name : value.english_name}</p>
                                <div style={{ fontSize: '0.75rem', margin: 0, display: 'flex', flexWrap: "wrap"}}>
                                    <span style={{ padding: '2px 5px', borderRadius: '5px',marginRight: 5, color: 'white', backgroundColor: '#f89e49', width: 40, height: 20, textAlign: "center"}}>{checkPermission}/{totalPermission}</span>
                                    <span style={{ fontSize: '15px'}}>{t("role_screen.role_selected")}</span>
                                </div>
                            </div>
                        )
                    })}
                    ...
                    <Grid container spacing={3}>
                        {arrayGroup.map(value => {
                            if(value.group == showGroup) {
                                ...
                                return (
                                    <Grid item xs={12} style={{ padding: '0px 15px', width: '100%'}}>
                                        <div style={{ float: 'left', fontWeight: "bold", fontSize: '1.2rem'}}>{ i18n.language == "vi" ? value.name : value.english_name}</div>
                                        <div style={{ float: 'right'}}>
                                            <Button style={{ color: '#fb9c45'}} onClick={() => checkGroup(value)}>
                                                {value.check == true ? t("role_screen.clear_all") : t("role_screen.select_all")}({totalPermission})</Button>
                                        </div>
                                    </Grid>
                                )
                            }
                            ...
                            if(value.group == showGroup) {
                                return (
                                    value.permission.map(role => {
                                        return (
                                            <Grid item xs={6}>
                                                <div style={{ padding: 10, backgroundColor: '#f6f6f6', width: '100%'}}>
                                                    <div style={{ display: 'flex', padding: '5px 0px', borderBottom: '#0000001f solid 1px'}}>
                                                        <Checkbox checked={role.check} onChange={() => checkTeam(role)} color={"primary"} />
                                                        <p style={{ paddingLeft: 10, fontWeight: 'bold'}}>{i18n.language == "vi" ? role.display_name : role.english_name}</p>
                                                    </div>
                                                    {role.permissions.map(role2 => {
                                                        return (
                                                            <div style={{ display: 'flex', padding: '5px 0px'}}>
                                                                <Checkbox checked={role2.check} onChange={() => checkRole(role2)} color={"primary"}/>
                                                                <p style={{ paddingLeft: 10}}>{i18n.language == "vi" ? role2.display_name : role2.english_name}</p>
                                                            </div>
                                                        )
                                                    })}
                                                </div>
