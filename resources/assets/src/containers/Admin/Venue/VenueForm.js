...
export default function (props) {
  ...
  const { data: sportDiscipline } = useFetch(["get", "/admin/sportDiscipline"]);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      formData.append(
        "training_place",
        trainingPlace ? JSON.stringify(trainingPlace) : JSON.stringify([])
      );
      formData.append(
        "competition_place",
        competitionPlace ? JSON.stringify(competitionPlace) : JSON.stringify([])
      );
      ...
      try {
        let res = await api.fetcher(
          "post",
          params.id ? "admin/venueCreate/" + params.id : "admin/venueCreate",
          formData
        );
        if (res) {
          history.push("/venue");
    ...
    validationSchema: Yup.object().shape({
      ...
      name: Yup.string()
        ...
        .nullable(),
      address: Yup.string()
        .required(t("venue_screen.address") + " " + t("errors.required"))
        .max(
          255,
        ...
        .nullable(),
      map_url: Yup.string()
        .required(t("venue_screen.map_url") + " " + t("errors.required"))
        .trim()
        .nullable(),
      latitude: Yup.string()
        .required(t("venue_screen.latitude") + " " + t("errors.required"))
        .max(
          255,
      ...
      longtitude: Yup.string()
        .required(t("venue_screen.longtitude") + " " + t("errors.required"))
        .max(
          255,
    ...
    <div>
      ...
      <Card style={{ padding: 20 }}>
              ...
              return (
                <tr>
                  ...
                  >
                    <Switch
                      color={"primary"}
                      checked={trainingPlace.includes(value.id) ? true : false}
                    />
                  </td>
                  <td
                    style={{
                  ...
                  >
                    <Switch
                      color={"primary"}
                      checked={
                        competitionPlace.includes(value.id) ? true : false
                      }
                    />
                  </td>
                </tr>
        ...
        <p style={{ color: "#f44336", fontSize: "0.75rem" }}>
          {api.error?.html || (formik.touched.html && formik.errors?.html)}
        </p>
          ...
          <Editor
            ...
            init={{
              ...
              images_upload_handler: function (blobInfo, success, failure) {
                ...
                xhr.open(
                  "POST",
                  process.env.MIX_REACT_APP_API_URL + "/admin/imageVenue"
                );
                xhr.onload = function () {
                  let json;
                  ...
                  temp = process.env.MIX_REACT_APP_PUBLIC_URL + temp;
                  success(temp);
                };
        ...
        <Dialog open={open} onClose={() => setOpen(false)} maxWidth="100%">
          <CancelIcon
            style={{
              color: "white",
              position: "fixed",
