...
export default function Score1vs1RoundIndividual({}) {
  ...
  useEffect(() => {
    if (match) {
      ...
      setMatchPointA(match?.match_individual_competitors[0]?.match_point);
      setMatchPointB(match?.match_individual_competitors[1]?.match_point);
      setQualificationA(
  ...
  const scoreA = score.filter(
    (e) =>
      (e?.competitor_id ? e?.competitor_id : e?.id) === member1?.competitor_id
  ).length;

  const scoreB = score.filter(
    (e) =>
      (e?.competitor_id ? e?.competitor_id : e?.id) === member2?.competitor_id
  ).length;

  const changeScoreReferee = (value, vlIndex, competitor_id) => {
    _setRound((pre) => [
      ...pre.map((e, rIndex) => {
        if (rIndex == vlIndex) {
          return {
            ...
            result: e?.result?.map((rl) => {
              if (rl?.competitor_id == competitor_id) {
                return { ...rl, resultScore: value ? value : 0 };
              }
              return rl;
            }),
          };
                          ...
                          >
                            <TextField
                              ...
                              value={
                                value?.result.find(
                                  (e) =>
                                    e?.competitor_id == member1?.competitor_id
                                )?.resultScore
                              }
                              size="small"
                              // variant={"outlined"}
                          ...
                          >
                            <TextField
                              ...
                              value={
                                value?.result.find(
                                  (e) =>
                                    e?.competitor_id == member2?.competitor_id
                                )?.resultScore
                              }
                              size="small"
                              // variant={"outlined"}
