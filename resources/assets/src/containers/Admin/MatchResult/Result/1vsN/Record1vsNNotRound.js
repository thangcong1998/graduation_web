...
export default function Record1vsNNotRound({ match, status }) {
      ...
      if (match?.match_individual_competitors?.length > 0) {
        match?.match_individual_competitors.forEach((value) => {
          ...
          if (value.final_score) {
            _member.finalScore = parseFloat(value.final_score);
          } else {
          ...
          if (value.result?.length > 0) {
            value.result.forEach((result) => {
              _member.score = parseFloat(result.score);
