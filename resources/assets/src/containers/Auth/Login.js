...
import anhnenpm from "../../../src/assets/image/anhnen.jpeg";
import iconSport from "../../../src/assets/image/sport.png";
...
export function FormLogin(props) {
  const classes = useStyle();
  const { t } = useTranslation();
  const { updateAdminToken, updateAdmin, updatePermission, admin } = useContext(
    AuthContext
  );
  const api = useAPI();
  const formik = useFormik({
...
export function DemoAutoPlay(props) {
  return (
          ...
          <div className={classes.contentRight}>
            ...
            >
              <img
                ...
                src={
                  `${iconSport}`
                  //   process.env.MIX_REACT_APP_STORAGE_URL + "/" + data?.logo_url
                }
              />
              {/* <div>Quản lý thể thao</div> */}
            </div>
            <div>
