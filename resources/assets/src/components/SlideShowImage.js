...
import { FormHelperText, Grid, IconButton, Button } from "@material-ui/core";
...
const UploadFileSlide = React.memo((props) => {
      ...
      if (file.id) {
        return (
          <button
            style={{
              backgroundImage: `url(${
                process.env.MIX_REACT_APP_STORAGE_URL + "/" + file.path
              })`,
            }}
  ...
  const { getRootProps, getInputProps, open } = useDropzone(
    !readOnly
      ...
      : { onDrop, noClick: true, noDrag: true, accept: "image/*" }
  );
  return (
    <div>
      <div
      ...
      >
        ...
        {readOnly !== true && (
          <div>
            <input {...getInputProps()} />
            <p
              onClick={(newFiles?.length > 0 || oldFiles?.length > 0) && open}
              className={"dragText"}
            >
              {newFiles.length === 0 &&
                oldFiles.length === 0 &&
